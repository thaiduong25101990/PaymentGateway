...
namespace BIDVWEB.Business
{
    public class SWIFT_TS_DAO
    {
        ...
        public static SWIFT_TS_DAO Instance()
		{
            return new SWIFT_TS_DAO();
		}

        public int Insert(SWIFT_TS_Info objTable)
        {
            int iResult;            
            string strSql = "GW_PK_SWIFT_TS.SWIFT_TS_Insert";
            try
            {
        ...
        public int Update(SWIFT_TS_Info objTable)
        {
            ...
            int iResult;
            string strSql = "GW_PK_SWIFT_TS.SWIFT_TS_Update";
        ...
        public DataSet SearchAdvance(string pWHERE, out DataSet _dtContent)
        {
            try
            {
                string strSql = "GW_PK_SWIFT_TS.Search";
                oraConn = objConn.Connect();
                if (oraConn == null)
                {
        ...
        public DataSet ViewSWIFT_TS(string pID, out DataSet _dtContent)
        {
            try
            {
                string strSql = "GW_PK_SWIFT_TS.ViewSWIFT_TS";
                oraConn = objConn.Connect();
                if (oraConn == null)
                {
                    return _dtContent = null;
        ...
        {
            string strSql = "GW_PK_SWIFT_TS.GetMSGByRM";
            oraConn = objConn.Connect();
            if (oraConn == null)
            {
                return _dtContent = null;
        ...
        {
            string strSql = "GW_PK_SWIFT_TS.CheckSubMSg";
        ...
        {
            string strSql = "GW_PK_SWIFT_TS.CheckMsgApprove";
            DataSet ds = new DataSet();
            oraConn = objConn.Connect();
            if (oraConn == null)
            {
        ...
        {
            string strSQL = "Select NVL(Max(to_number(substr(A.SBT_ID,6,9))),0) SBT_ID from SWIFT_TS A " +
                "WHERE SUBSTR(A.SBT_ID,6,6)=TO_CHAR(SYSDATE,'DDMMYY')";
            string sSBT_ID = "";
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                    if (ds.Tables[0].Rows[0]["SBT_ID"].ToString() != "0")
                        ...
                        sSBT_ID = "SWIFT" + DateTime.Now.Day.ToString().PadLeft(2, '0') +

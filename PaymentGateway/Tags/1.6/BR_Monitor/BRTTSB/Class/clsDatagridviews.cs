...
namespace BR.BRTTSB
{
    public class clsDatagridviews
    {
        ...
        public static DataGridView TTSP_MSG_CONTENT_LOAD(DataGridView _dtGridviews)
        {
            try
            {
                TTSP_MSG_CONTENTController ControlConten = new TTSP_MSG_CONTENTController();
                DataSet _dsContent;
                //----------------------------------------------------------------------------
                _dsContent = ControlConten.TTSP_CONTENT_LOAD(out _dsContent);
                ...
                //FomatGrid.Color_datagrid_TTSP(_dtGridviews);
        ...
        // ham lay du lieu day leb datatgrid dung chung
        public static DataGridView TTSP_MSG_CONTENT_RESEND(DataGridView _dtGridviews)
        {
            try
            {
                TTSP_MSG_CONTENTController ControlConten = new TTSP_MSG_CONTENTController();
        ...
        // ham lay du lieu day leb datatgrid dung chung
        public static DataGridView TTSP_MSG_CONTENT_SEARCH(DateTime _datefrom, DateTime _dateto, string pWhere, DataGridView _dtGridviews)
        {
            try
            {
                TTSP_MSG_CONTENTController ControlConten = new TTSP_MSG_CONTENTController();
                DataSet _dsContent;
                //----------------------------------------------------------------------------
                _dsContent = ControlConten.TTSP_CONTENT_SEARCH(_datefrom, _dateto, pWhere, out _dsContent);
                //DONGPV
                _dsContent.Tables[1].Merge(_dsContent.Tables[2]);
                _dsContent.Tables[0].Merge(_dsContent.Tables[1]);
                _dtGridviews.DataSource = _dsContent.Tables[0];
                //----------------------------------------------------------------------------
                clsEdit_columns_datagridview.Edit_Columns_Datagrid(_dtGridviews, _dsContent.Tables[3]);
        ...
        // ham lay du lieu day leb datatgrid dung chung
        public static DataGridView TTSP_MSG_CONTENT_SEARCH_RESEND(DateTime _datefrom, DateTime _dateto, string pWhere, DataGridView _dtGridviews)
        {
            try
            {
                TTSP_MSG_CONTENTController ControlConten = new TTSP_MSG_CONTENTController();
                DataSet _dsContent;
                //----------------------------------------------------------------------------
                _dsContent = ControlConten.TTSP_CONTENT_SEARCH_RESEND(_datefrom, _dateto, pWhere, out _dsContent);
                _dsContent.Tables[1].Merge(_dsContent.Tables[2]);
                _dsContent.Tables[0].Merge(_dsContent.Tables[1]);
                _dtGridviews.DataSource = _dsContent.Tables[0];
                //----------------------------------------------------------------------------
        ...
        // ham lay du lieu day leb datatgrid dung chung
        public static DataGridView TTSP_MSG_CONTENT_SEARCH_ADVANCE(string pWhere, DataGridView _dtGridviews)
        {
            try
            {
                TTSP_MSG_CONTENTController ControlConten = new TTSP_MSG_CONTENTController();
                DataSet _dsContent;
                //----------------------------------------------------------------------------
                _dsContent = ControlConten.TTSP_CONTENT_SEARCH_ADVANCE(pWhere, out _dsContent);                
                _dsContent.Tables[1].Merge(_dsContent.Tables[2]);
                _dsContent.Tables[0].Merge(_dsContent.Tables[1]);
                _dtGridviews.DataSource = _dsContent.Tables[0];                
                clsEdit_columns_datagridview.Edit_Columns_Datagrid(_dtGridviews, _dsContent.Tables[3]);//ok                
        ...
        // ham lay du lieu day leb datatgrid dung chung
        public static DataGridView TTSP_MSG_CONTENT_SEARCH_ADVANCE_RESEND(string pWhere, DataGridView _dtGridviews)
        {
            try
            {
                TTSP_MSG_CONTENTController ControlConten = new TTSP_MSG_CONTENTController();
                DataSet _dsContent;
                //----------------------------------------------------------------------------
                _dsContent = ControlConten.TTSP_CONTENT_SEARCH_ADVANCE_RESEND(pWhere, out _dsContent);
                _dsContent.Tables[1].Merge(_dsContent.Tables[2]);
                _dsContent.Tables[0].Merge(_dsContent.Tables[1]);
                _dtGridviews.DataSource = _dsContent.Tables[0];
                //----------------------------------------------------------------------------
        ...
        //DONGPV
        public static void Add_Combobox_TTSP(DataGridView _dtGrid)
        {
            try
            {
                //add cot processsts-------------------------------------------------------
                ...
                DataGridViewComboBoxColumn _cbProcesssts = new DataGridViewComboBoxColumn();
                _cbProcesssts.HeaderText = "STATUS_CODE";
                _cbProcesssts.DataSource = _ds.Tables[0];
                _cbProcesssts.DataPropertyName = "STATUS_CODE";
                _cbProcesssts.ValueMember = "STATUS_CODE";
                _cbProcesssts.DisplayMember = "NAME";
                _cbProcesssts.DisplayStyle = DataGridViewComboBoxDisplayStyle.Nothing;
                _dtGrid.Columns.Insert(_dtGrid.Columns["STATUS_CODE"].DisplayIndex, _cbProcesssts);
                _dtGrid.Columns["STATUS_CODE"].Visible = false;
                //add cot Error_code--------------------------------------------------------               
                DataGridViewComboBoxColumn _cbError_code = new DataGridViewComboBoxColumn();
                _cbError_code.HeaderText = "ERROR_CODE";
                _cbError_code.DataSource = _ds.Tables[1];
                ...
                _cbError_code.DisplayMember = "NAME";
                _cbError_code.DisplayStyle = DataGridViewComboBoxDisplayStyle.Nothing;
                _dtGrid.Columns.Insert(_dtGrid.Columns["ERROR_MESSAGE"].DisplayIndex, _cbError_code);
                _dtGrid.Columns["ERR_CODE"].Visible = false;
                //add cot Error_code--------------------------------------------------------   
            }
            catch (Exception ex)
            ...
            ComboBox cboStatus, DateTimePicker dateValue, bool _bol)
        {
            try
            {
                ...
                {
                    string pColumns = cbColumns.SelectedValue.ToString();
                    if (pColumns == "TRANSDATE" || pColumns == "SENDING_TIME" || pColumns == "RECEIVING_TIME" || pColumns == "VALUE_DATE")//neu search co dang datetime
                    {
                        string day = "";
                        string Month = "";
                    ...
                    else if (pColumns == "STATUS" || pColumns == "ERR_CODE" || pColumns == "FWSTS" || pColumns == "TAD")//phai search theo gia tri sealectedvalue
                    {
        ...
        public static String Search_Advance(DataGridView _dtgrs, out string datetimefrom, out string datetimeto, out string Direction)
        {
            try
            {
                string _dateFrom = ""; string _dateTo = ""; string _Direc = "";
                ...
                while (j < _dtgrs.Rows.Count)
                {
                    if (_dtgrs.Rows[j].Cells[0].Value != null)// hang duoc chon
                    {
                        if (_dtgrs.Rows[j].Cells[0].Value.ToString() == "True")
                        {
                            ...
                            else if (M[0].Trim() == "SENDER" || M[0].Trim() == "RECEIVER")
                            {
                                Where = Where + " and  Lpad("+M[0]+",5,'0')" + M[1] + "'" + M[2].Replace(",", "").Replace("'","") + "'";
                            }
                            ...
                            {
                                Where = Where + " and upper(ltrim(" + M[0] + ",'0000'))" + M[1] + "'" + M[2].Replace("'", "").Replace("''", "") + "'";
                            }
                            else if (M[0].Trim() == "MSG_DIRECTION")
                            {
                                _Direc = M[2];
                                Where = Where + " and upper(" + M[0] + ")" + M[1] + "'" + M[2].Replace("'", "").Replace("''", "") + "'";
                            }
                            else
                            {
                                Where = Where + " and upper(" + M[0] + ")" + M[1] + "'" + M[2].Replace("'", "").Replace("''", "") + "'";
        ...
        public static String Search_Normal(GroupBox _gr)
        {
            try
            {
                ...
                string Where_return = "";
                clsCheckInput clsCheck = new clsCheckInput();
                string strSource_branch = ""; string strMsg_type = "";
                string strSender = ""; string strcbtab = ""; string strHV_LV = "";
                string strRece = ""; string strRefno = ""; string strRMno = ""; string strAmount = ""; string strCurrency = "";
                string strDepartment = ""; string strMsg_dire = "";
                string strStatus = ""; string strResource = "";
                int k = 0;
                while (k < _gr.Controls.Count)
                {
                    ...
                    if (_gr.Controls[k] is TextBox)
                    {
                        ...
                        {
                            if (_gr.Controls[k].Text.Trim() == "") { strSender = ""; } else { strSender = " and    upper(Trim(SENDER)) like '%" + clsCheck.ConvertVietnamese(_gr.Controls[k].Text.Trim().Replace("'", "").Replace("''", "").ToUpper()) + "%'"; }
                        }
                        else if (_gr.Controls[k].Name == "txtreceiver")
                        {
                            if (_gr.Controls[k].Text.Trim() == "") { strRece = ""; } else { strRece = " and     upper(Trim(RECEIVER)) like '%" + clsCheck.ConvertVietnamese(_gr.Controls[k].Text.Trim().Replace("'", "").Replace("''", "").ToUpper()) + "%'"; }
                        }
                        ...
                        else if (_gr.Controls[k].Name == "txtRMno")
                        {
                            if (_gr.Controls[k].Text.Trim() == "") { strRMno = ""; } else { strRMno = " and upper(Trim(RM_NUMBER)) like '%" + clsCheck.ConvertVietnamese(_gr.Controls[k].Text.Trim().Replace("'", "").Replace("''", "").ToUpper()) + "%'"; }
                        }
                ...
                Where = strSource_branch + strMsg_type + strSender + strcbtab + strHV_LV + strRMno + strRece + strRefno + strAmount + strCurrency + strDepartment + strMsg_dire + strStatus + strResource;
                if (Where.Trim() == "")
                {
                    Where_return = "";
                }

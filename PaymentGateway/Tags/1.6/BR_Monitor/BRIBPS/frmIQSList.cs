    ...
    public partial class frmIQSList : Form
    {
        ...
        private void GET_CONTENT()
        {
            try
            {
                ...
                dsCONTENT = objControlCONTENT.GetIBPS_MSG_CONTENT_CONTENT(objInfoIQSList.MSG_ID.ToString(), strTable_name);
                ...
                if(dsCONTENT.Rows.Count==0)
                {
                    ...
                    dsCONTENT1 = objControlCONTENT.GetIBPS_MSG_CONTENT_CONTENT(objInfoIQSList.MSG_ID.ToString(), strTable_name1);
                    ...
                    if (dsCONTENT.Rows.Count == 0 && dsCONTENT1.Rows.Count == 0)
                    {
                        ...
                        dsCONTENT2 = objControlCONTENT.GetIBPS_MSG_CONTENT_CONTENT(objInfoIQSList.MSG_ID.ToString(), strTable_name2);
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (dgvIQSMessageList.Rows[f].Cells[0].Value != null)// hang duoc chon
                    {
                        if (dgvIQSMessageList.Rows[f].Cells[0].Value.ToString() == "True")
                        { iDem = iDem + 1; }
                    }                    
                    f = f + 1;
                }
                if (iDem == 0)
                { Common.ShowError("You must select rows! ", 3, MessageBoxButtons.OK); }
                        ...
                        while (d < dgvIQSMessageList.Rows.Count)
                        {
                            if (dgvIQSMessageList.Rows[d].Cells[0].Value != null)// hang duoc chon
                            {
                                if (dgvIQSMessageList.Rows[d].Cells[0].Value.ToString() == "True")
                                {
                                    ...
                                    objInfoIQSList.TOBANK = "00" + dgvIQSMessageList.Rows[d].Cells["RM_NUMBER"].Value.ToString().Substring(0, 3);
                                    objInfoIQSList.TELLER_ID = Common.Userid; 
                                    if (dgvIQSMessageList.Rows[d].Cells["TRANS_DATE"].Value.ToString() != "")
                                    {
                                        objInfoIQSList.ORG_TRANS_DATE = Convert.ToDateTime(dgvIQSMessageList.Rows[d].Cells["TRANS_DATE"].Value.ToString());
                                    }
                                    objInfoIQSList.INTERFACE = "IBPS"; 
        ...
        *--------------------------------------------------------------*/
        ...
        private void LoadData()
        {
            ...
            datDs = objControlIQSList.GetIQS_MSG_CONTENT("IBPS",listSelected);

    ...
    public class ProcessSend
    {
        ...
        *--------------------------------------------------------------*/
        ...
        public void ProcessService()
        {
            ...
            strSql = strSql + " t.gwtype,t.content, t.status,t.seq_no,  t.link_ref from TF_SIBS_MSG_IN t ";
            strSql = strSql + " Where t.status=0 ORDER BY t.LINK_REF, t.MSG_TYPE, t.REF_NO, t.SEQ_NO";
                ...
                if (ilen > 0)
                {
                    for (int i = 0; i < ilen; i++)
                    {
                        ...
                        try
                        {
                            ...
                            if (!Check_message_Send(lID, "SWIFT-SIBS"))
                            {
                                Insert_Index(lID, "SWIFT-SIBS");
                                if (m_strMsgIn == "MB68927I")
                                {
                                    // bIsSWIFT_TF = true;
                            ...
                            //else
                            //{
                            //    Lib.WriteLogDB(LOG_Warning, "Dien nay da duoc gui lan thu 2", m_lMsgID);
                            //}
                        }
                        catch //(Exception ex)
                        {
                        ...
                        if (i > 0 && (i % 5 == 0))
                            Thread.Sleep(10); // Cu 5 dien thi dat do tre 10 mili giay
            ...
            {
                Lib.WriteLogDB(LOG_Error, "Error When Get Data fron SIBS_MSG_IN" + ex.Message, 1);
            }
            finally
            {
                tblSIBSIN.Clear();
        ...
        public int Insert_Index(long LID, string MSG_DIRECTION)
        {
            ...
            try
            {
                string strSQL = "insert into TF_SVR_INDEX(MSG_ID,MSG_DIRECTION) values(:pLID,'SWIFT-SIBS')";
        ...
        public bool Check_message_Send(long LID, string MSG_DIRECTION)
        {
            ...
            try
            {
                ...
                string strSQL = "select MSG_ID from TF_SVR_INDEX TF where TF.MSG_ID= " + LID + "  and TF.MSG_DIRECTION ='" + MSG_DIRECTION + "' ";
        ...
        public DataTable CreateMessage(DataTable tblMSG_DEF, string strMessage)
        {
            ...
            try
            {
                ...
                while (i < tblMSG_DEF.Rows.Count)
                {
                    ...
                    if (strSIBSFieldcode == "SIMSNO")
                    {
                        if ((m_strOldMsgType == m_strMsgType) &&
                            (m_strOldRefNo == m_strRefNo) &&
                            (m_strOldLinkRef == m_strLinkRef))
                        {
                            strDetail = m_strMsgNo;
                        }
                    }
        ...
        *--------------------------------------------------------------*/
        ...
        public bool SendMessage(DataTable tblMSG_DEF, string strGW_TYPE, string strMBTransCodeOut)
        {
            ...
            try
            {
                ...
                if (cMessgeSock.ConvertToAS400_Table(g_strSIBSHost, g_strSIBSPort, tblMSG_DEF))
                {
                    ...
                    if (lErrorCode != 0)
                    {
                        ...
                        {
                            UpdateSWIFTContent(-1,6);
                        }
                        UpdateTFSIBSMsgIN(-1);
                        //UpdateSWIFTContent(-1);
                        string strLog = "Dien co MSG_ID: " + m_lMsgID + ", loai " + m_strMsgType + " voi kenh thanh toan " + m_strGWType + "  bi loi voi ma " + lErrorCode + szErr;
                    ...
                    else
                    {
                        ...
                        Lib.WriteLogDB(LOG_Info, strLog, m_lMsgID); //UpdateSWIFTContent(1);
                        //string strLog = "Dien co MSG_ID: " + m_lMsgID + ", loai " + m_strMsgType + " voi kenh thanh toan " + m_strGWType + "  da duoc gui thanh cong.";
                        //Lib.WriteLogDB(LOG_Info, strLog);
                    }
            ...
            catch (Exception ex)
            {
                UpdateTFSIBSMsgIN(-1);
                UpdateSWIFTContent(-1);
                Lib.WriteLogDB(LOG_Error, "Co loi trong ham Send cua SWIFT_TF. " + ex.Message, m_lMsgID);
                return false;
            }
        ...
        * Tham so          : string  strASCII, nooi dung dien nhan ve 
         *                 : bool bIsSWIFT_TF, Xac dinh la kenh thanh toan IBPS
        ...
        *--------------------------------------------------------------*/
        private void GetGWTYPE(string strGWType)
        {
            ...
            try
            {
                ...
                strSQL = "SELECT GT.SIBSIP, GT.PORTIN FROM GWSERVICE_PORT GT WHERE GT.SERVICENAME='" + SWIFTTFSend.SERVICE_NAME + "'";
        ...
        #region Ham Update du lieu vao bang TF_SIBS_MSG_IN
        /*---------------------------------------------------------------
        * Method           : UpdateTFSIBSMsgIN() 
        * Muc dich         : 
        *                  : 
        *                  : 
        ...
        *--------------------------------------------------------------*/
        ...
        private int UpdateTFSIBSMsgIN(int iStatus)
        {
            string strSql = "";
            try
            {
                strSql = "UPDATE TF_SIBS_MSG_IN Set Status=:pStatus Where ID=:pMsg_ID ";
                OracleParameter[] oraPara ={new OracleParameter("pStatus",OracleType.Int32), 
            ...
            {
                Lib.WriteLogDB(LOG_Error, "Update TF_SIBS_MSG_IN Error", m_lMsgID);
                return -1;
            }
        ...
        *--------------------------------------------------------------*/
        ...
        private int UpdateSWIFTContent(int status)
        {
            ...
            try
            {
                ...
                OracleParameter[] oraPara3 ={    new OracleParameter("pStatus",OracleType.Int32), 
                                                        new OracleParameter("pQueryID",OracleType.Int32)
                                                   };
                oraPara3[0].Value = status;
                oraPara3[1].Value = m_lMsgID;

                iResult = Lib.ExecuteNonQuery(strSql, CommandType.Text, oraPara3);

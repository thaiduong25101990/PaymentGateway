...
namespace BIDVWEB.Business
{
    public class VCB_TS_DAO
    {
        ...
        public static VCB_TS_DAO Instance()
		{
            return new VCB_TS_DAO();
		}

        public int Insert(VCB_TS_Info objTable)
        {
            int iResult;            
            string strSql = "GW_PK_VCB_TS.VCB_TS_Insert";
            try
            {
        ...
        public int Update(VCB_TS_Info objTable)
        {
            ...
            int iResult;
            string strSql = "GW_PK_VCB_TS.VCB_TS_Update";
        ...
        public DataSet SearchAdvance(string pWHERE, out DataSet _dtContent)
        {
            try
            {
                string strSql = "GW_PK_VCB_TS.Search";
                oraConn = objConn.Connect();
                if (oraConn == null)
                {
        ...
        public DataSet ViewVCB_TS(string pID, out DataSet _dtContent)
        {
            try
            {
                string strSql = "GW_PK_VCB_TS.ViewVCB_TS";
                oraConn = objConn.Connect();
                if (oraConn == null)
                {
                    return _dtContent = null;
        ...
        public DataSet GetMSGByRM(string sRM, string sField20, out DataSet _dtContent)
        {
            string strSql = "GW_PK_VCB_TS.GetMSGByRM";
            oraConn = objConn.Connect();
            if (oraConn == null)
            {
                return _dtContent = null;
            }
            OracleParameter[] Oraparam = {new OracleParameter("pRm_number",OracleType.NVarChar),
                                         new OracleParameter("pField20",OracleType.NVarChar),
                                         new OracleParameter("pContent",OracleType.Cursor)};
            Oraparam[0].Value = sRM;
            Oraparam[1].Value = sField20;
        ...
        {
            string strSql = "GW_PK_VCB_TS.CheckSubMSg";
        ...
        {
            string strSql = "GW_PK_VCB_TS.CheckMsgApprove";
            DataSet ds = new DataSet();
            oraConn = objConn.Connect();
            if (oraConn == null)
            {
        ...
        {
            string strSQL = "Select NVL(Max(to_number(substr(A.SBT_ID,4,9))),0) SBT_ID " +
                "from VCB_TS A WHERE SUBSTR(A.SBT_ID,4,6)=TO_CHAR(SYSDATE,'DDMMYY')";
            string sSBT_ID = "";            
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                    if (ds.Tables[0].Rows[0]["SBT_ID"].ToString()!="0")
                        ...
                        sSBT_ID = "VCB" + DateTime.Now.Day.ToString().PadLeft(2,'0') + 

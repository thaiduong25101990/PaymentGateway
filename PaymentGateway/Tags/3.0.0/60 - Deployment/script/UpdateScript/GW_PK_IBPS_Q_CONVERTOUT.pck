CREATE OR REPLACE PACKAGE GW_PK_IBPS_Q_CONVERTOUT is
  ...
  **********************************************************************/
  FUNCTION IBPS_CONVERTOUT(pQUERY_ID     NUMBER,
                           ...
                           --- 2014-09-17: Them field cho thue
                           content_tax out varchar2
                           --- Het them
                           ) RETURN VARCHAR2;
  ...
  **********************************************************************/
  ...
  procedure IBPS_QUERY_Update(pSIBS_MSG_ID NUMBER, pSTATUS NUMBER);
...
CREATE OR REPLACE PACKAGE BODY GW_PK_IBPS_Q_CONVERTOUT is
  ...
  BEGIN
    ...
    LOOP
      ...
      -- Ngay nghi van day dien OL4 & OL8 chi chan OL12, OL13
      if (isWorking_date > 0) AND
         ((substr(v_MSG_OUT.Content, 2166, 4) = 'OL12') or
         (substr(v_MSG_OUT.Content, 2166, 4) = 'OL13')) then
        goto continue_cursor;
      end if;
    
      --  Ngay Thu 7, Chu Nhat van cho dien di
      if (trim(upper(to_char(sysdate, 'Day'))) = 'SUNDAY' OR
         trim(upper(to_char(sysdate, 'Day'))) = 'SATURDAY') AND
         isWeekendWorking = 0 AND
         ((substr(v_MSG_OUT.Content, 2166, 4) = 'OL12') or
         (substr(v_MSG_OUT.Content, 2166, 4) = 'OL13')) then
        goto continue_cursor;
      end if;
      ...
      IF ((substr(v_MSG_OUT.Content, 2166, 4) = 'OL12') or
         (substr(v_MSG_OUT.Content, 2166, 4) = 'OL13')) and
         (to_date(to_char(sysdate, 'YYYYMMDD'), 'YYYYMMDD') -
         to_date(to_char(v_MSG_OUT.Trans_Date, 'YYYYMMDD'), 'YYYYMMDD') < 1) then
        goto continue_cursor;
      end if;
    ...
    While ((new_messages) and i < 1000) LOOP
      Begin
        ...
        BEGIN
          ...
          /*Quan LD them cho nang cap citad 2.1*/
        
          pRowIBPS.Desc_Tax := content_tax;
        
          pRowIBPS.Content_Tax := GW_PK_IBPS_DBLINK_CITAD_2_1.ConvertMSGTax(content_tax,
                                                                            GET_IBPS_Field(m_vCONTENT,
                                                                                           '031'),
                                                                            to_char(Sysdate,
                                                                                    'YYYYMMDD'),
                                                                            4,
                                                                            m_nErr);
          -- pRowIBPS.Product_Type    := substr(pCONTENT, 2165, 5);
          if m_nErr > 0 then
            pRowIBPS.Status := '-1';
          else
            /*--Minh
          ...
          if GETTAD_BY_NEW_RULE(pRowIBPS.F21,
                                pRowIBPS.F19,
                                pRowIBPS.F22,
                                isCMT) is null then
            pRowIBPS.Tad := m_vTAD;
            ...
            pRowIBPS.Tad := GETTAD_BY_NEW_RULE(pRowIBPS.F21,
                                               pRowIBPS.F19,
                                               pRowIBPS.F22,
                                               isCMT);
          end if;
          ...
          pRowIBPS.Product_Type := trim(substr(pCONTENT, 2166, 4));
          ...
          if (substr(pCONTENT, 2166, 4) = 'OL13') then
            pRowIBPS.Product_Type := 'OL13';
				  ...
				  -- them moi de nha ra dien msb TAx soan tu BDS
         if GET_IBPS_Field(m_vCONTENT, '033') ='7111' then
           pRowIBPS.Msg_Src :=8;
         end if;
         
         if GET_IBPS_Field(m_vCONTENT, '033') ='3492' then
           pRowIBPS.Msg_Src :=8;
         end if;
         -- Het them  
  ...
  **********************************************************************/
  ...
  FUNCTION IBPS_CONVERTOUT(pQUERY_ID     NUMBER,
                           pTRANS_DATE   DATE,
                           pMESSAGE_TEXT VARCHAR2,
                           pHeadContent  VARCHAR2, --- 2014-09-17: Them field cho thue
                           content_tax   out varchar2
                           --- Het them
                           ) RETURN VARCHAR2 IS
  ...
  BEGIN
    ...
    LOOP
      ...
      --- QuanLD: 2014-09-17: Them field cho thue
      if pvIBPSFilecode = '034' then
        content_tax := trim(pFieldContent);
      end if;
      --- Het them
      ...
      -- Minh.ha Cap nhat khi truong SoTK(33) va Truong ten TK(31)
      if pvIBPSFilecode = '033' or pvIBPSFilecode = '031' then
          ...
          IF (pvIBPSFilecode = m_vGW_RECEIVING_ADD_CODE) then
            ...
            if (pvIBPSFilecode = m_vGW_RECEIVING_BANK_CODE) Then
              ...
              if (pvIBPSFilecode = m_vGW_SEND_CORRES_CODE) then
                ...
                -- Map Citad
                m_vProxySender := IBPS_GETOPROXYCI(pQUERY_ID,
                                                   m_nAMOUNT,
                                                   m_receiveTruot,
                                                   iLV_HV);
    ...
    if (isCMT) then
      ...
      select count(1)
        into tmpCount
        from IBPS_TAD_MAP
       where BANK_CODE = substr(V019New, 3, 3);
      if (Length(V019New) > 6) and (tmpCount > 0) then
        select t.gw_bank_code
          into m_vProxySender
          from IBPS_TAD_MAP t
         where BANK_CODE = substr(V019New, 3, 3)
           and rownum = 1;
    ...
    --110: Relation Number
      WHEN (pFILECODE = m_vGW_RELNUM_CODE) then
       ...
       -- 20141104: QuanLD: Sua lai so but toan tu 6 so sang 8 so
        ...
        pReturn         := LPAD(m_nGW_TRANS_NUM, 8, 0);
  ...
  */ -------------------------------------------------------------------------------------------------------------------
  ...
  FUNCTION GETTAD_BY_NEW_RULE(vGWCode varchar2,
                              vF19    varchar2,
                              vF22    varchar2,
                              isCMT   Boolean) return varchar2 IS
    vReturn varchar2(20);
    i       number(20);
  BEGIN
    ...
    If (isCMT) then
      ...
      select count(1)
        into i
        from IBPS_TAD_MAP
       where BANK_CODE = substr(vF19, 3, 3)
         and rownum = 1;
    ...
    IF (ISlow = TRUE) then
      ...
      if (iCount > 0) then
        SELECT TAD.GW_BANK_CODE
          INTO vdaumoi
          FROM TAD
         WHERE substr(TAD.GW_BANK_CODE, 1, 2) =
               substr(pvBranchReceipt, 1, 2)

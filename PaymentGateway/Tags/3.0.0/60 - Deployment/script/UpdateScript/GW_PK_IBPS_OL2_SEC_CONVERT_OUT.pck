create or replace package GW_PK_IBPS_OL2_SEC_CONVERT_OUT is
  ...
  Function VCB_currency(pCCYCD varchar2) return number;	
	Function GetMsg_Src(pTellerId varchar2) return number;
end GW_PK_IBPS_OL2_SEC_CONVERT_OUT;
/
create or replace package body GW_PK_IBPS_OL2_SEC_CONVERT_OUT is
  ...
  Begin
    Begin
      select value
        into vcheckpoint
        from sysvar sysv
       where sysv.varname = 'IBPS_OL2_CHECKPOINT_SEC'
    ...
    begin
      select value
        into vcheckpoint1
        from sysvar sysv
       where sysv.varname = 'IBPS_OL2_CHECKPOINT_SEC1'
         and sysv.gwtype = 'IBPS'
         and rownum = 1;
    Exception
      when others then
  ...
  */
  Procedure ConvertIBPS_OL2 IS
    ...
    iCount             number(20) := 0;
    SEC_CutOff_time     number(10) := 1457;
    --
    ...
    Cursor vMSGOUT is
      Select *
       ...
       Where IO.STATUS = 0
         and Rownum < 100
         and IO.Attribute5 = 'SEC'
         And to_number(to_char(IO.Transdate, 'YYYYMMDDHH24MI')) <
             to_number(to_char(sysdate, 'YYYYMMDDHH24MI')) - 1
  ...
  BEGIN
    ...
    begin
      select to_number(value)
        into SEC_CutOff_time
        from sysvar
       where varname = 'IBPS_SEC_CUTOFF_TIME'
         and GWTYPE = 'IBPS'
         and rownum = 1;
    exception
      when others then
        SEC_CutOff_time := 1457;
    end;
    ...
    --if iCount > 0 then
      ...
      -- GW_UPDATE_MSG_REVERT.Get_ibps_sibs_ol2_cancel;
      -- Het update
--    end if;
    ...
    LOOP
      ...
      if (to_number(m_vMSGOUT.Attribute2) >
         to_number(to_char(sysdate, 'yyyyDDD') || '000000') +
         SEC_CutOff_time) then
        goto continue_cursor;
      end if;
      ...
      IF (GW_PK_IBPS_PROCESS.IBPS_MSG_DUP_CHECK(2, m_vMSGOUT.RM_NUMBER) > 0) then
        ...
        if CheckISVCB(m_vMSGOUT.Amount, vF22) = 0 then
          ...
          MSG_SRC       := GetMsg_Src(m_vMSGOUT.Tellerid);
          PRODUCT_TYPE  := 'OL2';
  ...
  Begin
    ...
    BEGIN
      SELECT SYSVAR.VALUE, SYSVAR.NOTE
        ...
        FROM SYSVAR
       WHERE SYSVAR.VARNAME = 'IBPSSIBSSeqNumOL2_SEC'
     ...
     WHERE VARNAME = 'IBPSSIBSSeqNumOL2_SEC'
       AND GWTYPE = 'IBPS';
    commit;
  ...
  BEGIN
    ...
    Begin
      ...
      m_vCONTENT := m_vF20 || m_vF32A || m_vF50K || m_vF53B || m_vF57D ||
                    m_vF57E || m_vF59 || m_vF70 || m_vF71A;
      ...
      pRowVCB.MSG_SRC           := GetMsg_Src(m_vTLBID);
  ...
  Function GetMsg_Src(pTellerId varchar2) return number is  
    
  begin
    if (pTellerId = 'CKVND001') then
      return 6;
    else
      return 7;
    end if;  

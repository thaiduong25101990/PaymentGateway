...
namespace BRIBPSInquiry
{
    public class ProcessInquiry
    {
        ...
        public static string m_strMsgNo = "";
        ...
        *--------------------------------------------------------------*/
        ...
        public void ProcessService()
        {
            ...
            tblGWTypeDept = Lib.ExcuteDataTable(strSQL);
            ...
            try
            {
                for (int i = 0; i < ilen; i++)
                {
                    ...
                    string GWType = tblGWTypeDept.Rows[i]["GWTYPE"].ToString().Trim();
                    string GWDept = tblGWTypeDept.Rows[i]["DEPARTMENT"].ToString().Trim();
                    // Goi ham Inquiry
            ...
            {
                Lib.WriteLogDB(LOG_Error, "IBPSInquiry failed." + ex.Message,1);
                return;
            }
        }
        #endregion
        ...
        *--------------------------------------------------------------*/
        ...
        public DataTable CreateInquiryDataTable(string strGWtype, string strDept)
        {
            ...
            {
                Lib.WriteLogDB(LOG_Error, "Send message to SIBS failed. " + ex.Message,1);
                throw (ex);
            }
        }
        #endregion
            ...
            try
            {
                ...
                try
                {
                    if (cMessgeSock.ConvertToAS400_Table(g_strSIBSHost, g_strSIBSPort, tblMSG_DEF))
                    {
                        ...
                        while (bMoreInQueue)
                        {
                            ...
                            for (int i = 0; i < iDetailNumber; i++)
                            {
                                ...
                                INSERT_IBPS_SIBS_QUERY(m_strMBTranscode_out, strGWtype, strDept, DateTime.Now, strMessageInsert);
                    ...
                    else
                    {
                        ...
                        Lib.WriteLogDB(LOG_Error, "Connect Server SIBS failed",1);
                    }
                }
                catch //(Exception ex)
                {
        ...
        *--------------------------------------------------------------*/
        private void GetGWTYPE_DEPT(string strGWType, string strDept)
        {
            ...
            try
            {
                ...
                strSQL = "SELECT GWP.SIBSIP,GWP.PORTIN FROM GWSERVICE_PORT GWP WHERE GWP.SERVICENAME='" + IBPSInquiry.SERVICE_NAME + "'";
                datTBGWType = new DataTable();
                datTBGWType = Lib.ExcuteDataTable(strSQL);
                if (datTBGWType == null)
                    throw (exp);
        ...
        *--------------------------------------------------------------*/
        ...
        /*---------------------------------------------------------------
        * Method           : INSERT_SIBS_QUERY() 
        * Muc dich         : Insert du lieu sao khi Query vao Database su dung PRE_INS_SIBS_QUEUE
        ...
        * Nguoi cap nhat   : QuanLD
        *--------------------------------------------------------------*/
        public int INSERT_IBPS_SIBS_QUERY(string pMBTrancode,
                                    string pGW_TYPE,
                                    string pGW_Dept,
                                    ...
                                    string pCONTENT)
        {
            ...
            {
                Lib.WriteLogDB(LOG_Error, " insert failed   SIBS_QEURY_PROCESS.PRE_INS_SIBS_QUEUE" + ex.Message,1);
                return -1;
            }
        }
        ...
        *--------------------------------------------------------------*/
        ...
        {
            bool isReturn = false;
            string strSQL = "SELECT Query_ID FROM IBPS_SIBS_QUERY Where DEPARTMENT='" + Dept + "' And Status=0 ";
            ...
            {
                Lib.WriteLogDB(2, " Get InitMemberVariable  IBPS_SIBS_Query failed" + ex.Message,1);
                isReturn = false;
            }
            return isReturn;

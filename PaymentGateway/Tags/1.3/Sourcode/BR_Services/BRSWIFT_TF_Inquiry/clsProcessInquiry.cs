 ...
 * ----------------------------
 * Noi dung: 
 * - Xay dung lop xu ly cho Service BRSWIFTTFInquiry
 ***************************************************************/
//=================================================================
...
namespace BRSWIFTTFInquiry
{
    public class ProcessInquiry
    {
        ...
        private string m_strGWType_Dept; // Luu ma 12 chu so cua kenh thanh toan nhu 000000000001
        ...
        *--------------------------------------------------------------*/
        ...
        public void ProcessService()
        {
            try
            {
                ...
                Inquiry(GWType, Department);
            }
            catch (Exception ex)
            {
                Lib.WriteLogDB(LOG_Error, SWIFTTFInquiry.SERVICE_NAME + " co loi xay ra khi Service Inquiry xu li." + ex.Message,1);
                return;
            }
        }
        #endregion
        ...
        *--------------------------------------------------------------*/
        ...
        public DataTable CreateInquiryDataTable(string strGWtype, string strDept)
        {
            ...
            {
                Lib.WriteLogDB(LOG_Error, "Loi khi gui dien vao SIBS. " + ex.Message,1);
                throw (ex);
            }
        }
        #endregion
            ...
            try
            {
                ...
                try
                {
                    if (cMessgeSock.ConvertToAS400_Table(g_strSIBSHost, g_strSIBSPort, tblMSG_DEF))
                    {
                        ...
                        while (bMoreInQueue)
                        {
                            ...
                            for (int i = 0; i < iDetailNumber; i++)
                            {
                                ...
                                INSERT_TF_SIBS_QUERY(m_strMBTranscode_out, strGWtype, strDept, DateTime.Now, strMessageInsert);
                    ...
                    else
                    {
                        ...
                        Lib.WriteLogDB(LOG_Error, "Khong ket noi duoc Server SIBS",1);
                    }
                }
                catch //(Exception ex)
                {
                    Lib.WriteLogDB(LOG_Error, "Khong gui nhan duoc Inquiry toi SIBS",1);
                }
        ...
        *--------------------------------------------------------------*/
        private void GetGWTYPE_DEPT(string strGWType, string strDept)
        {
            ...
            try
            {
                ...
                strSQL = "SELECT GWP.SIBSIP,GWP.PORTIN FROM GWSERVICE_PORT GWP WHERE GWP.SERVICENAME='" + SWIFTTFInquiry.SERVICE_NAME + "'";
                datTBGWType = new DataTable();
                datTBGWType = Lib.Select_ReturnDataTable(strSQL);
                if (datTBGWType == null)
                    throw (exp);
        ...
        *--------------------------------------------------------------*/
        ...
        #region Insert dien vao bang TF_SIBS_QUERY

        /*---------------------------------------------------------------
        * Method           : INSERT_TF_SIBS_QUERY() 
        * Muc dich         : Lay gia tri cua 1 field dien cos day du do dai theo chuan SIBS
        ...
        *--------------------------------------------------------------*/
        public int INSERT_TF_SIBS_QUERY(string pMBTrancode,
                                    string pGW_TYPE,
                                    string pGW_Dept,
                                    ...
                                    string pCONTENT)
        {
            ...
            {
                Lib.WriteLogDB(LOG_Error, "Co loi khi Insert data vao TF_SIBS_QUERY" + ex.Message,1);
                return -1;
            }
        }
        ...
        *--------------------------------------------------------------*/
        ...
        {
            bool isReturn = false;
            string strSQL = "SELECT Query_ID FROM TF_SIBS_QUERY Where GWTYPE= '"+GWType+"' and DEPARTMENT='" + Dept + "' And Status=0 ";
            DataTable tblQuery = new DataTable();

            try
            {
                tblQuery = Lib.Select_ReturnDataTable(strSQL);
            ...
            {
                Lib.WriteLogDB(2, "Co loi khi goi ham InitMemberVariable() kiem tra du lieu trong TF_SIBS_Query" + ex.Message,1);
                isReturn = true;
            }
            return isReturn;

  ...
  **********************************************************************/
  ...
  BEGIN
    ...
    LOOP
      ...
      If pvSIBSFileName = 'OTHER' then
        VOther := pFieldContent;
          ...
          --Cap nhat sua truong ngay cap CMT ngay 20120114
    ...
    if(isCMT) then
      ...
      select count(1) into tmpCount from IBPS_TAD_MAP where BANK_CODE=substr(V019,3,3);
      if (Length(V019)>6) and (tmpCount>0) then
        select t.gw_bank_code into m_vProxySender from IBPS_TAD_MAP t where BANK_CODE=substr(V019,3,3) and rownum=1 ;            
      End if;      

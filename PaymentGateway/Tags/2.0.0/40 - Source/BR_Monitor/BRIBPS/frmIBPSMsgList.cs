...
namespace BR.BRIBPS
{
    public partial class frmIBPSMsgList : BR.BRSYSTEM.frmBasedata
    {
         ...
         *--------------------------------------------------------------*/
        private void frmIBPSMsgList_Load(object sender, EventArgs e)
        {
            try
            {                
                ...
                //Ham lay du lieu----------------------------------- --------------------------------------
                dataMessage = clsDatagridviews.IBPS_MSG_CONTENT_LOAD(dataMessage);
        ...
        private void Searchdata()
        {
            try
            {
                ...
                {                  
                    ...
                    if (Where.Trim() != "")
                    { dataMessage = clsDatagridviews.IBPS_MSG_CONTENT_SEARCH_ADVANCE(Where, dataMessage); }//Lay du lieu theo dieu kien where                 
                ...
                else//tim kiem thong thuong
                {
                    ...
                    dataMessage.DataSource = 0;
                    label12.Text = "Total number of messages : 0";
                    dataMessage = clsDatagridviews.IBPS_MSG_CONTENT_SEARCH(datefrom.Value, dateto.Value, Where, dataMessage);//Lay du lieu theo dieu kien where                 
                    if (dataMessage.Rows.Count != 0) { _dtViews = (DataTable)dataMessage.DataSource; dsPrint = _dtViews; }
       ...
       *--------------------------------------------------------------*/
                ...
                if (txtValue.Visible == true)/*Dang hien thi textbox*/
                {
                    ...
                    else/*Khong su dung menh de like*/
                    {
                        if (Regex.IsMatch(txtValue.Text.Trim(), @"^[0-9]*\z") == true)/*Neu hoan toan la so*/
                        {
                            ...
                            if ((cbColumns.SelectedValue.ToString().Trim().ToUpper() == "F07" ||
                                cbColumns.SelectedValue.ToString().Trim().ToUpper() == "F19" ||
                                cbColumns.SelectedValue.ToString().Trim().ToUpper() == "F22" ||
                ...
                else
                {
                    ...
                    if (iDem == 0)
                    {
                        ...
                        if (frmSelectTypeIQS.isOK == true)
                        {
                            ...
                            IQSNew1.objInfoIQSNew1.TOBANK = dataMessage.CurrentRow.Cells["RM_NUMBER"].Value.ToString().Substring(0, 3); //dataMessage.CurrentRow.Cells["SOURCE_BRANCH"].Value.ToString();
                            ...
                            IQSNew1.objInfoIQSNew1.ORG_TRANS_DATE = Convert.ToDateTime(dataMessage.CurrentRow.Cells["TRANS_DATE"].Value.ToString());
                            IQSNew1.objInfoIQSNew1.TELLER_ID = Common.Userid;//dataMessage.CurrentRow.Cells["TELLERID"].Value.ToString();
                    ...
                    else if (iDem == 1)
                    {
                        ...
                        if (frmSelectTypeIQS.isOK == true)
                        {
                            ...
                            IQSNew1.objInfoIQSNew1.TOBANK = dataMessage.Rows[iRow].Cells["SOURCE_BRANCH"].Value.ToString();
                            ...
                            IQSNew1.objInfoIQSNew1.ORG_TRANS_DATE = Convert.ToDateTime(dataMessage.Rows[iRow].Cells["TRANS_DATE"].Value.ToString());
                            IQSNew1.objInfoIQSNew1.TELLER_ID = Common.Userid;//dataMessage.Rows[iRow].Cells["TELLERID"].Value.ToString();
                            IQSNew1.objInfoIQSNew1.MSG_TYPE = strMsg_Type;
        ...
        private void frmIBPSMsgList_KeyDown(object sender, KeyEventArgs e)
        {
            Common.bTimer = 1;
        }
        ...
        private void View_data(int Rows)
        {
            try
            {
                frmIBPSMsgInfo frmIbps = new frmIBPSMsgInfo();
                ...
                frmIbps.strPrin_STS = dataMessage.Rows[Rows].Cells["PRINT_STS"].Value.ToString();//SOURCE_BRANCH
                frmIbps.strSOURCE_BRANCH = dataMessage.Rows[Rows].Cells["SOURCE_BRANCH"].Value.ToString();
                frmIbps.dtPRINTSTS = _dsMaster.Tables["PRINT_STS"];
                frmIbps.strMSG_SRC = dataMessage.Rows[Rows].Cells["MSG_SRC"].Value.ToString();
                frmIbps.ShowDialog();
        ...
        private void frmIBPSMsgList_MouseMove(object sender, MouseEventArgs e)
        {
            Common.bTimer = 1;
        }

...
namespace BR.BRTTSB
{
    public partial class frmTTSPMsgList : BR.BRSYSTEM.frmBasedata
    {
        ...
        TTSP_MAP_FIELD_BOController objMapFiles = new TTSP_MAP_FIELD_BOController();
       ...
       *--------------------------------------------------------------*/
        ...
        private void View_data1(int Rows)
        {
            try
            {
                ...
                else
                {
                    ...
                    while (f < dataMessage.Rows.Count)
                    {
                        if (dataMessage.Rows[f].Cells[0].Value != null && dataMessage.Rows[f].Cells[0].Value.ToString() == "True")// hang khong duoc chon
                        {
                            ...
                            datRow = dsPrint.Select("MSG_ID=" + strMSG_ID);
                    ...
                    while (n < dataMessage.Rows.Count)
                    {
                        if (dataMessage.Rows[n].Cells[0].Value != null)// hang duoc chon
                        {
                            if (dataMessage.Rows[n].Cells[0].Value.ToString() == "True")// hang duoc chon
                            {
                                ...
                                string strMsgType = dataMessage.Rows[n].Cells["MSG_TYPE"].Value.ToString();

                                //STEP1: GET MAP COLUMNS:     
                                ArrayList arrMapColumns = LoadMapFields(strMsgType);

                                //STEP2: REVERT DATA FROM MSG_CONTENT TABLE TO TT_MT103 TABLE:
                                if (!RevertMsgContent(n))
                                {
                                    return;
                                }
                                
                                //STEP3: INSERT DATA FROM MSG_CONTENT TO TT_MT103:


                                //STEP4: CONVERT DATA FROM MSG TO TTSP MSG:
        ...
        #region RevertMsgContent

        public bool RevertMsgContent(int f)
        {
            try
            {
                if (dataMessage.Rows[f].Cells["MSG_TYPE"].Value.ToString().Trim() == "MT195" || dataMessage.Rows[f].Cells["MSG_TYPE"].Value.ToString().Trim() == "MT196")
                ...
                {
                    int QueryID = Convert.ToInt32(dataMessage.Rows[f].Cells["QUERY_ID"].Value);
                    string msgDirection  = dataMessage.Rows[f].Cells["MSG_DIRECTION"].Value.ToString();

                    if (msgDirection == "SIBS-TTSP")
                    {                        
                                                
                        //listSelected.Add(QueryID);
                    }
                    else if (msgDirection == "TTSP-SIBS")
                    {                        
        ...
        #region LoadMapFields

        public ArrayList LoadMapFields(string strMsgType)
        {
            try
            {
                DataSet dsMsgTxField = objMapFiles.GetMapFields(strMsgType);

                ArrayList arrMsgTxField = DataTable2ArrayList(dsMsgTxField.Tables[0]);

                return arrMsgTxField;
            }          
            catch (Exception ex)
            {
                Common.ShowError(ex.Message, 2, MessageBoxButtons.OK);
        ...
        public ArrayList DataTable2ArrayList(DataTable datatable)
        {
            try
            {
                ...
                {
                    System.Data.DataRow aRow = datatable.Rows[i];
                    ArrayList arrDetail = new ArrayList();

                    for (int j = 0; j < datatable.Columns.Count; j++)
                    {
                        arrDetail.Add(aRow[j]);
                    }
                ...
                else if (iSelect == 1)
                {
                    ...
                    while (f < dataMessage.Rows.Count)
                    {
                        if (dataMessage.Rows[f].Cells[0].Value != null && dataMessage.Rows[f].Cells[0].Value.ToString() == "True")// hang khong duoc chon
                        {
                            ...
                            datRow = _dtViews.Select("MSG_ID=" + strMSG_ID);

...
namespace BIDVWEB.Business.Reports
{
    ...
    {                
        ...
        {           
            ...
            ds = null;
            strValueReturn = "";
            //MinhHB tạo để check Status in trong trường hợp nhiều session cùng in
            int statusUpdate=0;
                ...
                if (dsData != null && dsData.Tables.Count > 0)
                {
                    ...
                    {                        
                        ...
                        if (CheckBranch_TAD(sBranch) > 0)
                        {
                            ...
                            if (objIBPS_BO.Check_Print_STS(objIBPS_Info) == 0)
                            {
                                ...
                                statusUpdate= objIBPS_BO.Update_Print_STS(objIBPS_Info);
                            }
                        }
                    }
                        ...
                        if (objSWIFT_Info.MSG_DIRECTION == "SIBS-SWIFT")
                        {
                            ...
                            if (sBranch.PadLeft(5, '0') == "00011")
                            {
                                if (objSWIFT_BO.Check_Print_STS(objSWIFT_Info) == 0)
                                {
                                    ...
                                    statusUpdate = objSWIFT_BO.Update_Print_STS(objSWIFT_Info);
                                }
                            }
                        }
                        //Dien den
                    ...
                    else if (strNR == "VCB_PRINT_1")
                    {
                        ...
                        if (objVCB_BO.Check_Print_STS(objVCB_Info) == 0)
                        {
                            ...
                            statusUpdate = objVCB_BO.Update_Print_STS(objVCB_Info);
                        }
                    }
                    ...
                    else if (strNR == "TTSP_PRINT_1")
                    {
                        ...
                        if (objTTSP_BO.Check_Print_STS(objTTSP_Info) == 0)
                        {
                            ...
                            statusUpdate = objTTSP_BO.Update_Print_STS(objTTSP_Info);
                        }
                    }
                    ...
                    //canhdm:end
                    //MinhHb: Check trang thai Update. Nếu Update=-2. Thì phải cập nhật lại trạng thái PrintStatus trong Dataset
                    if(statusUpdate==-2)
                        for(int i=0;i<ds.Tables[0].Rows.Count;i++)
                        {
                            ds.Tables[0].Rows[0]["PRINT_STS"]= 1;
                        }

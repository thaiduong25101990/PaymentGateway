...
namespace BRSWIFTTFMaintaince
{
    class ProcessMaintaince
    {
        ...
        public const string g_GWType = "SWIFT";
        public const string g_GWDept = "TF";
        ...
        *--------------------------------------------------------------*/
        ...
        public void ProcessService()
        {
            ...
            try
            {
                //Lib.WriteLogDB(LOG_Info, SWIFTTFMaintaince.SERVICE_NAME + " Bat dau tao cau Inquiry");
                string GWType = "SWIFT";
                string Department = "TF";
                Maintaince(GWType, Department);
            }
            catch (Exception ex)
            {
                Lib.WriteLogDB(LOG_Error, SWIFTTFMaintaince.SERVICE_NAME + " co loi xay ra khi Service Maintaince xu li." + ex.Message,1);
                return;
            } 
        ...
        *--------------------------------------------------------------*/
        ...
        public void Maintaince(string strGWtype, string strDept)
        {
            ...
            for (int i = 0; i < iCount; i++)
            {
                ...
                try
                {
                    // Dat trang thai Pending cho dien
                    UpdateQueryStatus(2); // Trang thai Pending
                    ...
                    if (cMessgeSock.ConvertToAS400_Table(g_strSIBSHost, g_strSIBSPort, tblMsg))
                    {
                        ...
                        if (iReceived <= 0)
                        {
                            Lib.WriteLogDB(LOG_Error, "SWIFTTF Maintaince - Message is empty", m_lMsgQueryID);
                            UpdateQueryStatus(0); // Cap nhat lai trang thai 0 de cho vong lap sau 
                            continue;
                        }
                        else
                        {
                            ...
                            strContent = strContent.TrimStart('%');

                            // Insert dien va cap nhat trang thai
                            // kiem tra lai truoc khi insert vao bang OUT
                           string strSQlQuery = "SIBS_QEURY_PROCESS.CheckQuery";
                            OracleParameter[] paramQuery = {
                                         new OracleParameter("pQuery_ID",OracleType.Number ,20),
                                         new OracleParameter("pDept",OracleType.NVarChar ,10),
                                         new OracleParameter("pireturn",OracleType.Number,3)};

                            paramQuery[0].Value = m_lMsgQueryID;
                            paramQuery[1].Value = "TF";
                            paramQuery[2].Direction = ParameterDirection.Output;
                            Lib.ExecuteNonQuery(strSQlQuery, CommandType.StoredProcedure, paramQuery);
                            if (Convert.ToInt32(paramQuery[2].Value) > 0)
                            {
                                UpdateQueryStatus(-2);
                            ...
                            {
                                if (InsertMessageContent(strHeadContent, strContent, strGWtype))
                                {
                                    UpdateQueryStatus(1);
                                }
                                else
                            ...
                            // Cap nhat ngay 21/08/2008
                            // Cap nhat lai trang thai cua dien tai HOST
                            tblMsg = new DataTable();
                            tblMsg = CreateMsgUpdateTable("MB68178I", "68178", "SWIFT", "TF");
                            if (cMessgeSock.ConvertToAS400_Table(g_strSIBSHost, g_strSIBSPort, tblMsg))
                            {
                                tblMsg.Dispose();
                                tblMSG_DEF = new DataTable();
                                tblMSG_DEF = GetMSG_DEF("MB68178R");

                                int headLength1 = GetGWHeadLength(m_SOCKHEAD, m_DSPHEAD, m_MBASEHEAD);
                                ...
                                unsafe
                                {
                                    ...
                                    int* DetailNumber = &iDetailNumber;
                                    // Nhan dien tu SIBS ve dien da duoc convert thanh kieu Ascii
                                    strASCII1 = cMessgeSock.ReceiveMessageASCII(tblMSG_DEF, m_strMBTranscode_Out, DetailStartPos, DetailLen, DetailNumber);
                                }
                                strASCII1 = strASCII1.Replace("\r%", "\r\n");
                            }
                            // Het cap nhat
                        }
                    }
                    ...
                    else
                    {
                        ...
                        // Neu chua gui duoc cap nhat lai trang thai la -1
                        UpdateQueryStatus(-1);
                        Lib.WriteLogDB(LOG_Error, "Can not request  SIBS Server", m_lMsgQueryID);
                    }
                ...
                {
                    UpdateQueryStatus(-1);
                    Lib.WriteLogDB(LOG_Error, "Error Maintaince service: " + ex.Message, m_lMsgQueryID);
                    continue;
                }
            }
        ...
        *--------------------------------------------------------------*/
        ...
        private bool InsertMessageContent(string strHeader, string Content, string strGwType)
        {
            try
            {
                ...
                string strSQL = "INSERT INTO TF_SIBS_MSG_OUT (Query_ID, Msg_Def, Trans_Date, Status, Head_Content,Content,GWTYPE) "
                            + " VALUES (:pQuery_ID,:pMsg_Def,:pTrans_Date,:pStatus,:pHeader,:pContent,:pGwtype)";
                OracleParameter[] param = {
                                         ...
                                         new OracleParameter(":pGwtype",OracleType.VarChar)
                                          };
                ...
                param[5].Value = Content.Replace("\n%", "\r\n");
                param[6].Value = strGwType;
        ...
        * Muc dich         : Cap nhan trang thai dien tai TF_SIBS_QUERY
        ...
        *--------------------------------------------------------------*/
        ...
        {
            string strSQL = "UPDATE TF_SIBS_QUERY SET Status = '" + iStatus + "' WHERE Query_ID = '" + m_lMsgQueryID + "'";
            try
            {
        ...
        *--------------------------------------------------------------*/
        ...
        #region CreateMsgUpdateTable - Tao DataTable de cap nhat voi TF chua ca Field de Inquiry

        /*---------------------------------------------------------------
        * Method           : CreateMsgUpdateTable() 
        * Muc dich         : Tao mot DataTable chua cac Field de cap nhat trang thai dien da nhan vao SIBS 
        *                  : Ham thuc hien goi mot ham de gui du lieu vao SIBS trong Dll
        * Tham so          :    string strGWtype,//Kenh thanh toan
        *                       string strDept //Phan he
        *                  
        ...
        *--------------------------------------------------------------*/
        ...
        public DataTable CreateMsgUpdateTable(string MBase_transcode, string strTransCode_Update, string strGWtype, string strDept)
        {
            int i = 0;//bien chay
            string strSIBSFieldcode;
            string strDefaulValue;
            ...
            try
            {
                GetUpdateGWTYPE_DEPT(strGWtype, strDept);
                tblMSG_DEF = GetMSG_DEF(MBase_transcode, false);
                while (i < tblMSG_DEF.Rows.Count)
                {
                    strDetail = "";
                    strSIBSFieldcode = tblMSG_DEF.Rows[i]["SIBS_FIELD_CODE"].ToString();
                    ...
                    iDatatype = Convert.ToInt32(tblMSG_DEF.Rows[i]["Data_Type"].ToString());
                    iSIBSPost = Convert.ToInt32(tblMSG_DEF.Rows[i]["SIBS_POS"].ToString());
                    strDetail = SetMemberValue_update(strTransCode_Update,strDefaulValue, strSIBSFieldcode, iLength, iGW_Post, iDatatype).ToString();//SetMemberValue(strDefaulValue, strSIBSFieldcode, iLength, iGW_Post, iDatatype).ToString();
                    strQueury = strQueury.ToString() + strDetail;
                    DataRow datRow = tblAS400.NewRow();
                    datRow["Content"] = strDetail;
                    datRow["GWpos"] = iGW_Post;
        ...
        *--------------------------------------------------------------*/
        ...
        public DataTable GetMsgList(string GWType, string Dept)
        {
            string strSQL = " SELECT QUERY_ID,RM_Number,Acct_Type,Seq3,Ref_Number,Msg_Type,Seq_No,Msg_No "
                          + " FROM TF_SIBS_QUERY WHERE STATUS=0 AND rownum <= 100 "
                          + " AND GWTYPE='" + GWType + "' AND DEPARTMENT='" + Dept + "' Order by Query_ID";
            return Lib.Select_ReturnDataTable(strSQL);
        }
        #endregion
         ...
         * Muc dich         : Xet cac gia tri vao noi dung cua dien
        ...
        * Nguoi cap nhat   : TrungNV
        *--------------------------------------------------------------*/
        public string SetMemberValue_update(string strTranCodeUpdate, string szContent, string szSIBSCode, int iFieldLength, int iFieldPos, long iDataType)
        {
            switch (szSIBSCode)
            {
                case GW_MSG_LEN:
        ...
        *--------------------------------------------------------------*/
        ...
        private void GetGWTYPE_DEPT(string strGWType, string strDept)
        {
            ...
            string strSQL = "select GD.FUNCTION_MAIN, GD.TRANS_CODE_MAIN_IN,GD.GWTYPE_DEPT,GD.MBTRANS_CODE_MAIN,GD.MBTRANS_CODE_MAIN_OUT, GD.NUMOFMSG from Gwtype_Dept GD "
            ...
            try
            {
                ...
                strSQL = "SELECT GT.SIBSIP, GT.PORTIN FROM GWSERVICE_PORT GT WHERE GT.SERVICENAME='" + SWIFTTFMaintaince.SERVICE_NAME + "'";
                datTBGWType = new DataTable();
                datTBGWType = Lib.Select_ReturnDataTable(strSQL);
                if (datTBGWType == null)
                    throw (exp);
        ...
        /*---------------------------------------------------------------
        * Method           : GetUpdateGWTYPE_DEPT() 
        * Muc dich         : Lay gia tri cua 1 field dien cos day du do dai theo chuan SIBS de Update cho TF
        * Tham so          : strSource gia tri truong dua vao
        *                  : iLength do dai chuoi lay ra
        *                  : iDatatype kieu du lieu trong SIBS cua chuoi kys tu dau vao
        *                  : iBeginPost Vi tri dau tien cua ky tu
        ...
        *--------------------------------------------------------------*/
        #region gan du lieu vao cac bien mac dinh m_strTransCode,m_ScenarioNum, m_strGWType_Dept, m_NumOfMsg
        private void GetUpdateGWTYPE_DEPT(string strGWType, string strDept)
        {
            ...
            DataTable datTBGWType = new DataTable();
            string strSQL = "select GD.FUNCTION_SEND, GD.GWTYPE_DEPT from Gwtype_Dept GD "
                          + " where GD.GWTYPE =  '" + strGWType + "'   and GD.DEPARTMENT = '" + strDept + "'";
            try
            {
                datTBGWType = new DataTable();
                ...
                if (datTBGWType.Rows.Count > 0)
                {
                    m_strScenarioNum = m_strGWType_Dept = datTBGWType.Rows[0]["FUNCTION_SEND"].ToString();
                    m_strTranCode = "68178";
                    m_strGWType_Dept = m_strGWType_Dept = datTBGWType.Rows[0]["GWTYPE_DEPT"].ToString();
                    m_strMBTranscode = "MB68178I";
                    m_strMBTranscode_Out = "MB68178R";

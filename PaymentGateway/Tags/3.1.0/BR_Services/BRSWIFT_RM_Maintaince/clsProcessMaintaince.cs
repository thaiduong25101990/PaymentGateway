...
namespace BRSWIFTRMMaintaince
{
    class ProcessMaintaince
    {
        ...
        *--------------------------------------------------------------*/
        ...
        public void ProcessService()
        {
            ...
            string[] GWTypes = new string[] { "SWIFT", "IBS" };
            string GWDept = "RM";
            foreach (string GWType in GWTypes)
            {
                try
                {
                    Maintaince(GWType, GWDept);
                }
                catch //(Exception ex)
                {
                ...
                Thread.Sleep(100); // Moi phan he nghi 100 mili giay
            }
        }
        #endregion
            ...
            for (int i = 0; i < iCount; i++)
            {
                ...
                try
                {
                    ...
                    if (cMessgeSock.ConvertToAS400_Table(g_strSIBSHost, g_strSIBSPort, tblMsg))
                    {
                        ...
                        if (iReceived <= 0)
                        {
                            Lib.WriteLogDB(LOG_Error, "SWIFTRM Maintaince - Message is empty",m_lMsgQueryID);
                    ...
                    else
                    {
                        ...
                        // Neu chua gui duoc cap nhat lai trang thai la 0
                        UpdateQueryStatus(-1);
        ...
        *--------------------------------------------------------------*/
        ...
        private bool InsertMessageContent(string strHeader, string Content,string  strGwtype)
        {
            try
            {
                string strSQL = "INSERT INTO RM_SIBS_MSG_OUT (Query_ID, Msg_Def, Trans_Date, Status, Head_Content,Content,GWTYPE) "
        ...
        * Muc dich         : Cap nhan trang thai dien tai TRM_SIBS_QUERY
        ...
        *--------------------------------------------------------------*/
        ...
        {
            string strSQL = "UPDATE RM_SIBS_QUERY SET Status = '" + iStatus + "' WHERE Query_ID = '" + m_lMsgQueryID + "'";
            try
            {
        ...
        *--------------------------------------------------------------*/
        ...
        public DataTable CreateMsgTable(string strGWtype, string strDept)
        {
            ...
            {
                Lib.WriteLogDB(LOG_Error, "Error when connect to SIBS. " + ex.Message, m_lMsgQueryID);
                throw (ex);
            }
        }
        #endregion
        ...
        *--------------------------------------------------------------*/
        ...
        public DataTable GetMsgList(string GWType, string Dept)
        {
            string strSQL = " SELECT QUERY_ID,RM_Number,Acct_Type,Seq3,Ref_Number,Msg_Type,Seq_No,Msg_No "
                          + " FROM RM_SIBS_QUERY WHERE STATUS=0 AND rownum <= 5  AND GWTYPE='" + GWType + "'"
                          + " AND DEPARTMENT='" + Dept + "'";
            return Lib.Select_ReturnDataTable(strSQL);
        }
        #endregion
        ...
        *--------------------------------------------------------------*/
        ...
        private void GetGWTYPE_DEPT(string strGWType, string strDept)
        {
            ...
            string strSQL = "select  GD.GWTYPE_DEPT,GD.FUNCTION_MAIN, GD.TRANS_CODE_MAIN_IN,GD.MBTRANS_CODE_MAIN,GD.MBTRANS_CODE_MAIN_OUT, GD.NUMOFMSG from Gwtype_Dept GD "
            ...
            try
            {
                ...
                if (datTBGWType.Rows.Count > 0)
                {
                    ...
                    m_strMsgDef = GetMBTRANSCODEOUT(m_strMsgType);
                    if (m_strMsgDef.Trim() == "")
                    {
                        m_strMsgDef = m_strMBTranscode_Out;
                    }
                    ...
                    m_iMsgSize = GetSIBSLength(m_SOCKHEAD, m_DSPHEAD, m_MBASEHEAD, m_strMBTranscode);
                    m_strMBTranscode_Out = m_strMsgDef;
                }
                datTBGWType.Clear();
                // ket thuc -----------------------------------------
                ...
                strSQL = "SELECT GT.SIBSIP, GT.PORTIN FROM GWSERVICE_PORT GT WHERE GT.SERVICENAME='" + SWIFTRMMaintaince.SERVICE_NAME + "'";
                datTBGWType = new DataTable();
                datTBGWType = Lib.Select_ReturnDataTable(strSQL);
                if (datTBGWType == null)
                    throw (exp);
    ...
    * Nguoi cap nhat   : QuanLD
    *--------------------------------------------------------------*/
        private string GetMBTRANSCODEOUT(string strMSG_TYPE )
        {
            string  MBASECodeOut="";
            //strConnString = getConnString();
            DataTable datTBGWType = new DataTable();
            string strMSGTYPETemp = "";
            if (strMSG_TYPE.Substring(3, 1) == "9")
            {
                strMSGTYPETemp = strMSG_TYPE.Substring(0, 2).ToString() + "1" + strMSG_TYPE.Substring(3).ToString();

            }
            else
            { 
                strMSGTYPETemp = strMSG_TYPE; 
            }

            string strSQL = "SELECT SIBS_TRANS_CODE FROM SWIFT_MSG_MAP WHERE MAP_DIRECTION=1 AND Rownum =1 And SWIFT_TRANS_CODE= '" + strMSGTYPETemp + "'";
            ...
            try
            {
                ...
                if (datTBGWType == null)
                    throw (exp);
                if (datTBGWType.Rows.Count > 0)
                    MBASECodeOut = datTBGWType.Rows[0]["SIBS_TRANS_CODE"].ToString();
                datTBGWType.Clear();
                datTBGWType.Dispose();
                return MBASECodeOut;
            }

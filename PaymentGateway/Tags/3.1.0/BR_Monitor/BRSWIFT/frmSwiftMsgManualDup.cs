...
{
    public partial class frmSwiftMsgManualDup : Form
    {
        ...
        private void cmdOK_Click(object sender, EventArgs e)
        {
            ...
            {         
                ...
                if (objControlContent.UPDATE_CLOSE_MESSAGE(cboStatus.SelectedValue.ToString(), strQUERY_ID, strTable_name) == 1)
                {
                    if (cboStatus.SelectedValue.ToString() == Common.PROCESSSTS_FAILED)
                    {
                        MessageBox.Show("Failed message successfully!", Common.sCaption, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                    }
                    else
                    {
                    ...
                    if (cboStatus.SelectedValue.ToString() == Common.PROCESSSTS_FAILED)
                    {
                        objSWIFT_log.DESCRIPTIONS = Common.Userid + ": Failed message successfully";
                    }
                    else
                    {
                        objSWIFT_log.DESCRIPTIONS = Common.Userid + ": Closed message successfully";
                    }
                    objcontrolSWIFT_log.SELECT_SWIFT_MSG_LOG(objSWIFT_log);
                ...
                else
                {
                    if (cboStatus.SelectedValue.ToString() == Common.PROCESSSTS_FAILED)
                    {
                        MessageBox.Show("Failed message error!", Common.sCaption, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
                    ...
                    objSWIFT_log.QUERY_ID = Convert.ToInt32(strQUERY_ID);
                    objSWIFT_log.STATUS = -1;
                    objSWIFT_log.DESCRIPTIONS = Common.Userid + ": Closed or Failed message error";
                    objcontrolSWIFT_log.SELECT_SWIFT_MSG_LOG(objSWIFT_log);
                    objcontrolSWIFT_log.ADD_SWIFT_MSG_LOG(objSWIFT_log);
                }
        ...
        private void frmSwiftMsgManualDup_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        {            
            ...
            else if (dsGroup.Tables[0].Rows.Count == 1)
            {
                ...
                if (IsSupervisor == 1)//Teller
                {
                    ...
                    if (pProcess == Common.PROCESSSTS_WAITING || pProcess == Common.PROCESSSTS_NACK || pProcess == Common.PROCESSSTS_OLD_ACKWAIT || pProcess == Common.PROCESSSTS_FAILED)
                    {
                        cmdOK.Enabled = true;
                        cboStatus.Enabled = true;
                    }
        ...
        private void frmSwiftMsgManualDup_KeyDown(object sender, KeyEventArgs e)
        {
            Common.bTimer = 1;
            if (Common.iCancel == 0)
            {
        ...
        private void frmSwiftMsgManualDup_MouseMove(object sender, MouseEventArgs e)
        {
            Common.bTimer = 1;
        }

        private void frmSwiftMsgManualDup_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (iClose == false)
            {
                if (Common.iCancel == 0)
        ...
        private void Swift_redend()
        {
            try
            {
                if (objControlContent.Resend_message_swift_tc(strQUERY_ID, strTable_name) == 1)
                {
                    ...
                    objSWIFT_log.QUERY_ID = Convert.ToInt32(strQUERY_ID);
                    objSWIFT_log.STATUS = 1;
                    objSWIFT_log.DESCRIPTIONS = Common.Userid + ": Send swift successfully";
                    objcontrolSWIFT_log.ADD_SWIFT_MSG_LOG(objSWIFT_log);
                    Refresh_log();
                    iClose = true;
                    this.Close();
        ...
        private void frmSwiftMsgManualDup_MouseDown(object sender, MouseEventArgs e)
        {
            Common.bTimer = 1;
        }
        ...
        private void cmdApprove_Click(object sender, EventArgs e)
        {
            try
            {
                if (objControlContent.Resend_message_swift_tc(strQUERY_ID, strTable_name) == 1)
                {
                    MessageBox.Show("Send imported to swift successfully!", Common.sCaption, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                    objSWIFT_log.LOG_DATE = DateTime.Now;
                    objSWIFT_log.QUERY_ID = Convert.ToInt32(strQUERY_ID);
                    objSWIFT_log.STATUS = 1;
                    objSWIFT_log.DESCRIPTIONS = Common.Userid + ": Send imported to swift successfully";
                    objcontrolSWIFT_log.ADD_SWIFT_MSG_LOG(objSWIFT_log);
                    iClose = true;
                    Common.iCancel = 0;
                    objContent.OK = "False";
                ...
                {
                    MessageBox.Show("Send imported to swift error!", Common.sCaption, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    objSWIFT_log.LOG_DATE = DateTime.Now;
                    objSWIFT_log.QUERY_ID = Convert.ToInt32(strQUERY_ID);
                    objSWIFT_log.STATUS = -1;
                    objSWIFT_log.DESCRIPTIONS = Common.Userid + ": Send imported to swift error";
                    objcontrolSWIFT_log.ADD_SWIFT_MSG_LOG(objSWIFT_log);
                }
            }
            catch (Exception ex)
        ...
        private void cmdReject_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Reject imported successfull!", Common.sCaption, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                    objSWIFT_log.LOG_DATE = DateTime.Now;
                    objSWIFT_log.QUERY_ID = Convert.ToInt32(strQUERY_ID);
                    objSWIFT_log.STATUS = 1;
                    objSWIFT_log.DESCRIPTIONS = Common.Userid + ": Reject imported successfull";
                    objcontrolSWIFT_log.ADD_SWIFT_MSG_LOG(objSWIFT_log);
                    iClose = true;
                    Common.iCancel = 0;
                    objContent.OK = "False";
                ...
                {
                    MessageBox.Show("Reject imported error!", Common.sCaption, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    objSWIFT_log.LOG_DATE = DateTime.Now;
                    objSWIFT_log.QUERY_ID = Convert.ToInt32(strQUERY_ID);
                    objSWIFT_log.STATUS = -1;
                    objSWIFT_log.DESCRIPTIONS = Common.Userid + ": Reject imported error";
                    objcontrolSWIFT_log.ADD_SWIFT_MSG_LOG(objSWIFT_log);
                }

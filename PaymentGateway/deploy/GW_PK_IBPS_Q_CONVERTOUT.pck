CREATE OR REPLACE PACKAGE GW_PK_IBPS_Q_CONVERTOUT is
  ...
  **********************************************************************/
  ...
  -- Map tu dong
  FUNCTION RECEIPVER_BANK_MAP(vF19 Varchar2,fieldCode number) return varchar2;

  FUNCTION GETTAD_BY_NEW_RULE(vGWCode varchar2,vF19 varchar2,vF22 varchar2,isCMT Boolean) return varchar2;

  FUNCTION IBPS_GETOPROXYCI(pQUERY_ID NUMBER, pAmount number, pvBranchReceipt varchar2, ISlow  in out boolean) RETURN VARCHAR2;
...
CREATE OR REPLACE PACKAGE BODY GW_PK_IBPS_Q_CONVERTOUT is
  ...
  BEGIN
    ...
    LOOP
      ...
      if (isWorking_date > 0) AND ((substr(v_MSG_OUT.Content,2166,4)='OL12') or (substr(v_MSG_OUT.Content,2166,4)='OL13')) then
         goto continue_cursor;
      end if;
      
      --  Ngay Thu 7, Chu Nhat van cho dien di
      if (trim(upper(to_char(sysdate, 'Day'))) = 'SUNDAY' OR trim(upper(to_char(sysdate, 'Day'))) = 'SATURDAY') AND isWeekendWorking=0 AND ((substr(v_MSG_OUT.Content,2166,4)='OL12') or (substr(v_MSG_OUT.Content,2166,4)='OL13')) then
         goto continue_cursor;
      end if; 
